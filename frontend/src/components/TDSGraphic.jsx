    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <motion.div
          ...
          className="bg-white rounded-xl p-4 border-2 border-[#4A6C6F]"
        >
          ...
          <div className="text-2xl text-slate-900">{matchedCount}</div>
        ...
        <div className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200">
          ...
          <div className="text-2xl text-slate-900">{mismatchCount}</div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="text-sm text-slate-900">{record.vendor}</div>
                  <div className="text-xs text-slate-600">
                    Invoice: ₹{record.amount.toLocaleString()} • TDS: ₹{record.tds.toLocaleString()}
                  </div>
                </div>

                {record.status === 'matched' ? (
                  <div className="text-xs bg-slate-100 text-slate-700 px-3 py-1 rounded-full border border-slate-300">
                    Verified
                  </div>
                ) : (
                  <div className="text-xs bg-red-100 text-red-700 px-3 py-1 rounded-full border border-red-300">
                    ₹{Math.floor(record.tds * 0.02)} variance
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <div className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-slate-900">Reconciliation Progress</span>
        ...
        <div className="mt-2 text-xs text-slate-600">
          AI matching with 26AS form • {matchedCount}/{records.length} auto-verified
        </div>
      </div>
    </motion.div>
  );
