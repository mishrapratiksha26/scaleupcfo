  ...
  useEffect(() => {
    const stageInterval = setInterval(() => {
      setMatchingStage((prev) => {
        if (prev >= 3) {
          setCollected((current) => [...current, activePayment]);
    ...
    >
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-lg text-slate-900">
              {payments[activePayment].customer}
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="text-xs text-slate-600">
              {payments[activePayment].invoice}
            </div>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-sm text-slate-900">
                    {payments[paymentIndex].customer}
                  </div>
                  <div className="text-xs text-slate-600">
                    {payments[paymentIndex].invoice}
                  </div>
                </div>
                <div className="text-sm text-slate-900">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-slate-50 rounded-xl p-2.5 border border-slate-200 text-center">
          <div className="text-lg text-slate-900">{collected.length + 47}</div>
